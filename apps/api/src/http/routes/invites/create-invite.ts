...
export async function createInvite(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().register(auth).post('/organizations/:slug/invites', {
    ...
    if (cannot('create', 'Invite')) {
      throw new UnauthorizedError('You are not allowed to create a invite in this organization');
    }
    ...
    if (organization.shouldAttachUsersByDomain && organization.domain === domain) {
      throw new BadRequestError('You are not allowed to invite users with the same domain as the organization');
    }
    ...
    if (inviteWithSameEmail) {
      throw new BadRequestError('Another invite with this email already exists');
    }
