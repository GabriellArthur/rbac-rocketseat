...
export async function getOrganizationBilling(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().register(auth).get('/organizations/:slug/billing', {
    schema: {
      tags: ['billing'],
      summary: 'Get an organization billing',
      security: [{ bearerAuth: [] }],
      params: z.object({
        slug: z.string(),
      }),
    ...
    const { organization, membership } = await request.getUserMembership(slug.toLowerCase());

    const { cannot } = getUserPermissions(userId, membership.role);

    if (cannot('get', 'Billing')) {
      throw new UnauthorizedError('You are not allowed to get the billing of this organization');
    }
